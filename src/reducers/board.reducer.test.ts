...
import { ADD_MOVE, AddMoveAction, RESET_BOARD } from "../interfaces/actions.i";

const baseState = boardReducer(undefined, { type: "@test/unknown" } as AddMoveAction);
...
describe("boardReducer", () => {
  ...
  it("stores an updated board after a move", () => {
    const nextBoard = ["X", 1, 2, 3, 4, 5, 6, 7, 8] as AddMoveAction["tiles"];
  ...
  it("resets the board back to empty tiles", () => {
    const played = boardReducer(baseState, {
      type: ADD_MOVE,
      tiles: ["X", "O", "X", "O", "X", "O", "O", "X", "O"] as AddMoveAction["tiles"],
    });
